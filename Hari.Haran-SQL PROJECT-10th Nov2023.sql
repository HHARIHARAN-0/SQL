...
-- SQL Project Questions and Answers  -----------------------





-- Q.1. List Top 3 products based on QuantityAvailable. (productid, productname,QuantityAvailable ). (3 Rows) [Note: Products]--
...
-- Q.2. Display EmailId of those customers who have done more than ten purchases. (EmailId, Total_Transactions). (5 Rows) [Note: Purchasedetails, products] --
...
select pdt.EmailId,  count(pdt.ProductId) as Total_Transactions
from purchasedetails pdt
inner join products pd on pdt.productId = pd.productId
group by pdt.EmailId
having count(pdt.productId) > 10
...
-- Q.3. List the Total QuantityAvailable category wise in descending order. (Name of the category, QuantityAvailable) (7 Rows) [Note: products, categories] --
...
select ct.categoryid, sum(pd.QuantityAvailable) from categories ct inner join products pd 
on ct.categoryid = pd.categoryid
group by ct.categoryid
order by sum(pd.quantityavailable) desc limit 7;
...
-- Q.4. Display ProductId, ProductName, CategoryName, Total_Purchased_Quantity for the product which has been sold maximum in terms of quantity? (1 Row) [Note: purchasedetails, products, categories] --
...
SELECT pd.ProductId, pd.ProductName, ct.CategoryName, SUM(pdt.QuantityPurchased) AS Total_Purchased_Quantity
FROM purchasedetails pdt
INNER JOIN products pd ON pdt.ProductId = pd.ProductId
INNER JOIN categories ct ON pd.CategoryId = ct.CategoryId
GROUP BY pd.ProductId, pd.ProductName, ct.CategoryName
...
-- Q.5. Display the number of male and female customers in fastkart. (2 Rows) [Note: roles, users] --
...
select roleid, gender from users;

select gender, count(*) as Total_Counts
from users
group by gender;
...
-- Q.6. Display ProductId, ProductName, Price and Item_Classes of all the products where Item_Classes are as follows:
...
from products order by price desc; #displaying in highest to lowest price
...
-- Q. 7. Write a query to display ProductId, ProductName, CategoryName, Old_Price(price) and New_Price as per the following criteria
...
select pd.productid, pd.productname, ct.categoryname, pd.price,
case categoryname
...
from products pd inner join categories ct ON pd.categoryid = ct.categoryid order by productid;
...
-- Q.8. Display the percentage of females present among all Users. (Round up to 2 decimal places) Add “%” sign while displaying the percentage. (1 Row) [Note: users]

describe users;

select concat(round( count(gender='F') / (select count(gender) from users)*100, 2 ), ' ' '%')
as Total_Females
from users
where gender ='F';
...
-- Q.9. Display the average balance for both card types for those records only where CVVNumber > 333 and NameOnCard ends with the alphabet “e”. (2 Rows) [Note: carddetails]

describe carddetails;

select cardtype, cvvnumber, balance from carddetails;
...
-- Q.10. What is the 2nd most valuable item available which does not belong to the “Motor” category. 
...
select pd.productname, pd.price, pd.QuantityAvailable, ct.categoryname 
from products as pd
...
select pd.productname, ct.categoryname, (pd.Price * pd.QuantityAvailable) as Value
from products as pd
inner join categories as ct
on pd.categoryid = ct.categoryid
where categoryname != 'motors'
